...
--        directly
function TSP:InsertNode(nodes, metadata, zoneID, nodeID, radius, insertOptions)
	assert(type(nodes) == "table", "InsertNode() expected table in 1st argument, got "..type(nodes).." instead.")
	insertOptions = insertOptions or {}
	local nearestOnly = insertOptions.nearest_only
	local maxClosestDistance = tonumber(insertOptions.max_closest_distance) or 0
	local skipIfFar = insertOptions.skip_if_far
	...
	local nearestIndex, nearestDistance = 1, inf
	for i = 1, numNodes-1 do
		local d = weight[i*numNodes-numNodes]
		if d < nearestDistance then
			nearestDistance = d
			nearestIndex = i
		end
	end
	if skipIfFar and maxClosestDistance > 0 and nearestDistance > maxClosestDistance then
		tremove(nodes)
		return TSP:PathLength(nodes, zoneID)
	end
	...
	local insertPoint
	local allowNearestOnly = nearestOnly and numNodes > 3
	local function edgeAllowed(a, b)
		if allowNearestOnly and a ~= nearestIndex and b ~= nearestIndex then
			return false
		end
		if maxClosestDistance > 0 and weight[a*numNodes-numNodes] > maxClosestDistance and weight[b*numNodes-numNodes] > maxClosestDistance then
			return false
		end
		return true
	end
	...
	local wrapAllowed = edgeAllowed(numNodes-1, 1)
	local wrapGain = weight[(numNodes-1)*numNodes-numNodes] + weight[numNodes*numNodes-1] - weight[(numNodes-1)*numNodes-1]
	if wrapAllowed and wrapGain < shortestPathLength then
		...
		if not insertPoint then
			local left = nearestIndex == 1 and numNodes-1 or nearestIndex-1
			local right = nearestIndex == numNodes-1 and 1 or nearestIndex+1
			local leftGain = weight[left*numNodes-numNodes] + weight[numNodes*numNodes-nearestIndex] - weight[left*numNodes-nearestIndex]
			local rightGain = weight[nearestIndex*numNodes-numNodes] + weight[numNodes*numNodes-right] - weight[nearestIndex*numNodes-right]
			insertPoint = rightGain < leftGain and (nearestIndex + 1) or nearestIndex
		end
		if metadata then
			...
			if try2 > numNodes-1 then
				try2 = 1
			end
			if weight[try1*numNodes-numNodes] > weight[numNodes*numNodes-try2] then
				try1, try2 = try2, try1
			end
			local flag = tryInsert(nodes, metadata, try1, nodeID, radius, zoneW, zoneH)
			if not flag then
