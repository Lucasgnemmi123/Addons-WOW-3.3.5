...
local defaults = {
	global = {
		...
		defaults = {            --    r,    g,    b,   a
			...
			cluster_dist = 60,
			auto_insert_nearest_only = true,
			auto_insert_max_closest_distance = 0,
			auto_insert_skip_if_far = false,
			auto_foreground_on_insert = true,
			callbacks = {
				['*'] = true
			}
		},
...
function Routes:DrawWorldmapLines()
	...
	local routesInMap = db.routes[mapFile]
	if not routesInMap then return end
	for route_name, route_data in pairs(routesInMap) do
...
local XY_cache_mt = {
	__index = function(t, key)
		...
		if cache_zone ~= zone then
			local zoneKey = tonumber(zone) or zone
			cache_zoneW, cache_zoneH = Routes.mapData:MapArea(zoneKey)
			cache_zone = zone
		end
...
function Routes:DrawMinimapLines(forceUpdate)
	...
	local zoneInfo = zone and self.LZName[zone]
	if (not WorldMapFrame or not WorldMapFrame:IsShown()) and zoneInfo and zoneInfo[3] and zoneInfo[4] then
		SetMapZoom(zoneInfo[3], zoneInfo[4])
	elseif (not WorldMapFrame or not WorldMapFrame:IsShown()) and type(SetMapToCurrentZone) == "function" then
		SetMapToCurrentZone()
	end
	...
	if not _x or not _y or _x < 0 or _x > 1 or _y < 0 or _y > 1 then
		if (not WorldMapFrame or not WorldMapFrame:IsShown()) and type(SetMapToCurrentZone) == "function" then
			SetMapToCurrentZone()
	...
	local zoneMapFile = self.LZName[zone][1]
	local zoneKey = currentZoneID
	local zoneW, zoneH = self.mapData:MapArea(currentZoneID)
	if not zoneW or zoneW == 0 then
		zoneW, zoneH = self.mapData:MapArea(zoneMapFile)
		zoneKey = zoneMapFile
	end
	if not zoneW or zoneW == 0 then
		return
	...
	if currentZoneID ~= GetCurrentMapAreaID() and (not WorldMapFrame or not WorldMapFrame:IsShown()) and zoneInfo and zoneInfo[3] and zoneInfo[4] then
		SetMapZoom(zoneInfo[3], zoneInfo[4])
		_x, _y = GetPlayerMapPosition("player")
		if _x and _y and _x >= 0 and _x <= 1 and _y >= 0 and _y <= 1 then
			cx, cy = zoneW * _x, zoneH * _y
		end
	end
	...
	local routesInZone = db.routes[mapFile]
	if not routesInZone then return end
	for route_name, route_data in pairs(routesInZone) do
		if type(route_data) == "table" and type(route_data.route) == "table" and #route_data.route > 1 then
			-- store color/width
			...
			if (not route_data.hidden and (route_data.visible or not defaults.use_auto_showhide)) or defaults.show_hidden then
				...
				if route_data.looped and route_data.route[ #route_data.route ] ~= defaults.fake_point then
					local key = format("%s;%s", zoneKey, route_data.route[ #route_data.route ])
				...
				for i = 1, #route_data.route do
					...
					if point == defaults.fake_point then
						...
						local key = format("%s;%s", zoneKey, point)
...
local function GetAutoInsertOptions()
	return {
		nearest_only = db.defaults.auto_insert_nearest_only,
		max_closest_distance = db.defaults.auto_insert_max_closest_distance,
		skip_if_far = db.defaults.auto_insert_skip_if_far,
	}
end

local function RunAutoForegroundOptimize(mapFile, route_data)
	if not db.defaults.auto_foreground_on_insert then return false end
	if not route_data or not route_data.route then return false end
	if #route_data.route > 724 then return false end

	local running, runningRoute = Routes.TSP:IsTSPRunning()
	if running and runningRoute == route_data.route then
		return false
	end
	...
	for tabooname, used in pairs(route_data.taboos) do
		if used then
			tinsert(taboos, db.taboo[mapFile][tabooname])
		end
	end

	local output, meta, length = Routes.TSP:SolveTSP(route_data.route, route_data.metadata, taboos, Routes.mapData:MapAreaId(mapFile), db.defaults.tsp)
	route_data.route = output
	route_data.length = length
	route_data.metadata = meta
	return true
end
			...
			for k, v in pairs(route_data.selection) do
				if k == node_name or v == node_name then
					...
					if flag then
						...
						local oldLength = route_data.length
						route_data.length = self.TSP:InsertNode(route_data.route, route_data.metadata, self.LZName[zone][2], coord, route_data.cluster_dist or 65, GetAutoInsertOptions()) -- 65 is the old default
						if route_data.length ~= oldLength then
							RunAutoForegroundOptimize(mapFile, route_data)
							throttleFrame:Show()
						end
					end
					break
...
function Routes:OnInitialize()
	...
	-- Force line visibility defaults for legacy clients.
	db.defaults.draw_minimap = true
	db.defaults.draw_worldmap = true
	db.defaults.draw_indoors = true
	db.defaults.use_auto_showhide = false
...
options.args.routes_group.args.auto_insert_settings = {
	type = "group",
	name = "Auto-insert behavior",
	order = 120,
	inline = true,
	args = {
		auto_insert_nearest_only = {
			type = "toggle",
			name = "Attach new nodes to nearest route points",
			desc = "If enabled, automatic node insertion prefers segments connected to the closest existing route point.",
			order = 10,
			get = function() return db.defaults.auto_insert_nearest_only end,
			set = function(info, v) db.defaults.auto_insert_nearest_only = v end,
		},
		auto_insert_max_closest_distance = {
			type = "range",
			name = "Max nearest distance (yards)",
			desc = "If greater than 0, nodes farther than this from the closest route point are ignored when auto-inserting.",
			min = 0, max = 1000, step = 5,
			order = 20,
			get = function() return db.defaults.auto_insert_max_closest_distance end,
			set = function(info, v) db.defaults.auto_insert_max_closest_distance = v end,
		},
		auto_insert_skip_if_far = {
			type = "toggle",
			name = "Skip auto-insert when too far",
			desc = "Only used when max nearest distance is greater than 0.",
			order = 30,
			disabled = function() return (db.defaults.auto_insert_max_closest_distance or 0) <= 0 end,
			get = function() return db.defaults.auto_insert_skip_if_far end,
			set = function(info, v) db.defaults.auto_insert_skip_if_far = v end,
		},
		auto_foreground_on_insert = {
			type = "toggle",
			name = "Re-optimize in foreground after each new node",
			desc = "Runs the same foreground optimizer automatically whenever a new node is inserted into a route.",
			order = 40,
			get = function() return db.defaults.auto_foreground_on_insert end,
			set = function(info, v) db.defaults.auto_foreground_on_insert = v end,
		},
	},
}
...
do
	...
	function Routes:UnTabooRoute(zone, route_data)
		for i = #route_data.taboolist, 1, -1 do
			...
			if flag == false then
				local oldLength = route_data.length
				route_data.length = Routes.TSP:InsertNode(route_data.route, route_data.metadata, Routes.mapData:MapAreaId(zone), coord, route_data.cluster_dist or 65, GetAutoInsertOptions()) -- 65 is the old default
				if route_data.length ~= oldLength then
					tremove(route_data.taboolist, i)
					throttleFrame:Show()
				end
