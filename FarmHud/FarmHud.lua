...
local function GetGatherDisplayModule()
	if GatherMate2 and GatherMate2.GetModule then
		return GatherMate2:GetModule("Display")
	end
	if GatherMate and GatherMate.GetModule then
		return GatherMate:GetModule("Display")
	end
	return nil
end
...
local options = {
	...
	args = {
		...
		show_gathermate = {
			type = "toggle", width = "double",
			name = "Toggle GatherMate/GatherMate2 support",
			order = 6,
...
local onShow = function(self)
	...
	local gatherDisplay = GetGatherDisplayModule()
	if gatherDisplay and (FarmHudDB.show_gathermate == true) then
		gatherDisplay:ReparentMinimapPins(FarmHudMapCluster)
		gatherDisplay:ChangedVars(nil, "ROTATE_MINIMAP", "1")
	end
	...
	if Routes and Routes.ReparentMinimap and (FarmHudDB.show_routes == true) then
		...
		if Routes.DrawMinimapLines then
...
local onHide = function(self, force)
	...
	local gatherDisplay = GetGatherDisplayModule()
	if gatherDisplay then
		gatherDisplay:ReparentMinimapPins(Minimap)
		gatherDisplay:ChangedVars(nil, "ROTATE_MINIMAP", fh_mapRotation)
	end
