...
local function waypointsEnabled()
	return db and db.defaults and db.defaults.enable_waypoints
end
...
function Waypoints:FindClosestVisibleRoute()
	...
	local zoneInfo = Routes.LZName and Routes.LZName[zone]
	if not zoneInfo or zoneInfo[1] == "" then
		return
	end
	local mapfile = zoneInfo[1]
	local zoneRoutes = db.routes[mapfile]
	if type(zoneRoutes) ~= "table" then
		return
	end
	local closest_zone, closest_route, closest_node
	local min_distance = 1/0
	local defaults = db.defaults
	for route_name, route_data in pairs(zoneRoutes) do  -- for each route in current zone
...
function Waypoints:QueueFirstNode()
	if not waypointsEnabled() then
		Routes:Print("Waypoints desactivado. Routes muestra lineas en minimapa/mapa.")
		return
	end
	if not hasActiveCartographerWaypoints() then
		Routes:Print(L["Cartographer_Waypoints module is missing or disabled"])
	...
	if a then
		...
		local mapfile = zoneInfo[1]
		if type(db.routes[mapfile]) ~= "table" then
			return
		end
		route_table = db.routes[mapfile][b]
		if type(route_table) ~= "table" or type(route_table.route) ~= "table" then
			return
		end
		node_num = c
...
function Waypoints:OnInitialize()
	db = Routes.db.global
	if db.defaults.enable_waypoints == nil then
		db.defaults.enable_waypoints = false
	end
...
options = {
	...
	args = {
		enable_waypoints = {
			name = "Enable Waypoint Arrow",
			desc = "Activa/desactiva la flecha de Cartographer_Waypoints. Si esta desactivado, Routes solo dibuja lineas.",
			type = "toggle",
			order = 5,
			get = function() return db.defaults.enable_waypoints end,
			set = function(info, v)
				db.defaults.enable_waypoints = v
				if not v then
					Waypoints:RemoveQueuedNode()
				end
			end,
		},
		desc = {
